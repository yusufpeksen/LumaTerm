...
test('language setting defaults safely and round-trips',()=>{assert.equal(settings().language,'tr');assert.equal(settings({language:'en'}).language,'en');assert.equal(settings({language:'invalid'}).language,'tr');});
