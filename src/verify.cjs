...
module.exports=async({app,win,sessions,store})=>{
  ...
  assert.equal(await win.webContents.executeJavaScript('Boolean(document.getElementById("suggestions-button"))'),false);
  assert.equal(await win.webContents.executeJavaScript('Boolean(document.getElementById("footer-metrics"))'),true);
  ...
  assert.equal(await win.webContents.executeJavaScript('Boolean(document.querySelector("input[name=suggestions]"))'),false);
  ...
  await fs.writeFile(path.join(app.getPath('userData'),'package-check.json'),JSON.stringify({passed:true,version:app.getVersion(),checks:['packaged renderer','packaged ConPTY DLL','real PowerShell output','colored Git-aware prompt','PowerShell directory tracking','single-context file explorers','file filtering','session metrics','English default UI','DPAPI encryption']},null,2));
