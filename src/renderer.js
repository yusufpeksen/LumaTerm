...
let state, active, split = false, panelVisible = true, localDir, remoteDir = '/', remoteEntries = [], localEntries = [], remoteRequest = 0, localRequest = 0;
...
async function openSession(spec) {
  ...
  try {
    ...
    term.open(pane);
    s.suggestions=new Suggestions(s,{enabled:()=>state.settings.suggestions,send:data=>api.input(s.id,data),isActive:()=>active===s.id&&!$('modal').open});
    term.onData(data=>{api.input(s.id,data);s.suggestions.input(data);});
    ...
    term.attachCustomKeyEventHandler(event=>{
      ...
      if(!s.suggestions.key(event))return false;
...
function updateTabs(){
  $('tabs').innerHTML=[...tabs.values()].map(s=>msg`<button class="tab ${s.id===active?'active':''}" data-id="${s.id}">${s.kind==='ssh'?icon('server-cog'):shellIcon(s.spec.shell||'powershell')}<span>${esc(s.title)}${s.exited?tr(' · kapalı'):''}</span><span class="tab-close" title="Sekmeyi kapat">${icon('x')}</span></button>`).join('');
  $('tabs').querySelectorAll('.tab').forEach(b=>{b.onclick=()=>activate(b.dataset.id);b.querySelector('.tab-close').onclick=run(e=>{e.stopPropagation();return closeSession(b.dataset.id);});}); paintIcons();
}
function activate(id){ for(const s of tabs.values())s.suggestions?.hide();active=id; updateTabs(); layout(); updateSessionInfo(); current()?.term.focus(); }
function layout(){
  ...
  $('file-panel').hidden=!panelVisible; $('split-button').style.color=split?'var(--accent)':'';
  scheduleFit();
}
...
function updateSessionInfo(){
  ...
  const s=current(); $('session-info').textContent=s?`${s.kind==='ssh'?'SSH':'LOCAL'}  /  ${s.cwd||s.title}`:'';
...
async function closeSession(id){const s=tabs.get(id);if(!s)return;if(state.settings.confirmClose&&!s.exited&&!await ask(tr('Oturum kapatılsın mı?'),s.title+tr(' oturumundaki çalışan işlemler sonlanır.')))return;await api.sessionClose(id);s.suggestions.dispose();s.term.dispose();s.pane.remove();tabs.delete(id);if(active===id)active=[...tabs.keys()].at(-1);updateTabs();layout();updateSessionInfo();updateFooter();}
function eventReceived(e){
  if(e.type==='notice'){toast(e.message,true);return;}
  if(e.type==='transfer'){transfers.set(e.transferId,e);renderTransfer();return;}
...
function renderTransfer(){const t=[...transfers.values()].at(-1);$('transfer-status').textContent=t?`${transfers.size>1?transfers.size+tr(' aktarım · '):''}${t.name} · ${t.total?Math.round(t.done/t.total*100):0}%`:'';}
...
async function settingsModal(){
  ...
  const suggestionNote=document.createElement('div');suggestionNote.className='info-box';suggestionNote.textContent=tr('Ctrl+Space önerileri açar, Ctrl+→ seçileni ekler. Tab kabuğun tamamlamasını kullanır. Geçmiş yalnızca bu oturumda tutulur.');$('settings-form').append(suggestionNote);
...
function fileRows(entries,remote){return entries.map((e,i)=>`<div class="file-row ${e.directory?'directory':''}" draggable="true" data-name="${esc(e.name)}" title="${esc(e.name)} · ${new Date(e.mtime).toLocaleString(getLanguage()==='en'?'en-US':'tr-TR')}">${icon(fileIcon(e))}<span class="file-name">${esc(e.name)}${e.symlink?' ↗':''}</span><span class="file-size">${e.directory?'—':size(e.size)}</span>${remote?msg`<span class="file-tools"><button data-action="download" title="İndir">${icon('download')}</button><button data-action="drag" title="Explorer’a sürüklemek için hazırla">${icon('external-link')}</button><button data-action="rename" title="Yeniden adlandır">${icon('pencil')}</button><button data-action="delete" title="Sil">${icon('trash-2')}</button></span>`:''}</div>`).join('')||tr('<div class="empty-note">Bu klasör boş.</div>');}
async function loadRemote(dir=remoteDir){const s=ssh(),request=++remoteRequest;$('remote-files').innerHTML=tr('<div class="empty-note">Dosyalar yükleniyor…</div>');try{const entries=await api.filesList(s.id,dir);if(request!==remoteRequest||s.id!==active)return;remoteDir=dir;s.remoteDir=dir;remoteEntries=entries;$('remote-path').value=dir;s.fileEntries=entries;s.fileDirectory=dir;remoteView.set(sortedFiles(entries));}catch(e){if(request===remoteRequest)$('remote-files').innerHTML=`<div class="empty-note danger">${esc(e.message)}</div>`;throw e;}}
async function loadLocal(dir=localDir){const id=active,request=++localRequest;const entries=await api.filesList(null,dir);if(request!==localRequest||id!==active)return;localDir=dir;if(current())current().localDir=dir;localEntries=entries;$('local-path').value=dir;if(current()){current().fileEntries=entries;current().fileDirectory=dir;}localView.set(sortedFiles(entries));}
function bindFiles(remote){const root=$(remote?'remote-files':'local-files');root.querySelectorAll('.file-row').forEach(row=>{
  ...
  row.ondblclick=run(()=>entry.directory?(remote?loadRemote(full):loadLocal(full)):remote?download([full],sessionId):undefined);
...
async function init(){
  ...
  on('files-toggle',()=>{panelVisible=!panelVisible;layout();});
  on('terminal-search-button',searchTerminal);on('search-close',()=>{$('terminal-search').hidden=true;current()?.term.focus();});
  on('suggestions-button',()=>{current()?.term.focus();current()?.suggestions.update(true);});
